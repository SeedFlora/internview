...
public interface CompanyRequestRepository extends JpaRepository<CompanyRequest, Long> {
    ...
    @Query("SELECT cr FROM CompanyRequest cr WHERE (:cursor IS NULL OR cr.companyRequestId > :cursor) AND LOWER(cr.companyName) LIKE LOWER(CONCAT('%', :search, '%')) ORDER BY cr.companyRequestId ASC")
    List<CompanyRequest> findPageBySearchFromCursor(@Param("search") String search, @Param("cursor") Long cursor, Pageable pageable);

    @Query("SELECT cr FROM CompanyRequest cr WHERE cr.status = :status AND (:cursor IS NULL OR cr.companyRequestId > :cursor) AND LOWER(cr.companyName) LIKE LOWER(CONCAT('%', :search, '%')) ORDER BY cr.companyRequestId ASC")
    List<CompanyRequest> findPageByStatusAndSearchFromCursor(@Param("status") CompanyRequestStatus status, @Param("search") String search, @Param("cursor") Long cursor, Pageable pageable);
}
