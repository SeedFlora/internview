...
public interface CompanyRepository extends JpaRepository<Company, Long> {
    ...
    boolean existsByCompanySlugAndCompanyIdNot(String companySlug, Long companyId);
    ...
    @Query(value = """
        ...
        WHERE :cursor IS NULL
           OR (COALESCE(cr.avg_rating, 0) < COALESCE(
                   (SELECT cr2.avg_rating FROM company_ratings cr2 WHERE cr2.company_id = :cursor), 0))
           OR (COALESCE(cr.avg_rating, 0) = COALESCE(
                   (SELECT cr2.avg_rating FROM company_ratings cr2 WHERE cr2.company_id = :cursor), 0)
