...
public class ReviewService implements IReviewService {
    ...
    // Must match the review form's "maks. 4" limit (StepInfo.jsx disables the
    // subcategory picker at 4). Was 3, so a user who legitimately picked the
    // 4th subcategory the UI allowed got a ValidationException on submit.
    private static final int MAX_SUB_CATEGORIES = 4;
    ...
    private void validateLookupExists(String lookupCode, String lookupValue) {
        // BUG FIX: this only checked that the lookup CODE (category) existed and
        // ignored the submitted lookupValue entirely, so bogus values like
        // internshipType="ZZZ" or workScheme="FAKE" passed validation and were
        // stored, later rendering as blank/garbage on the company page. Verify
        // the value is one of the allowed values for the code.
        List<Lookup> lookups =
                lookupRepository.findAllByLookupTypeAndLookupCode(LOOKUP_TYPE_INTERNSHIP_REVIEW, lookupCode);

        if (lookups.isEmpty()) {
            throw new ValidationException(String.format("Invalid lookup code: %s", lookupCode));
        }

        boolean valueAllowed = lookups.stream()
                .anyMatch(l -> l.getLookupValue().equals(lookupValue));
        if (!valueAllowed) {
            throw new ValidationException(
                    String.format("Invalid value '%s' for %s", lookupValue, lookupCode));
        }
    }
