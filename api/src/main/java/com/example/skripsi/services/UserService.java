...
public class UserService implements IUserService {
    ...
    public CertificateResponse reviewCertificateRequest(Long requestId, ReviewCertificateRequest request) {
        ...
        // BUG FIX: status was an unvalidated free string compared with ==APPROVED.
        // Anything else (e.g. lowercase "approved", or a typo) silently took the
        // REJECTED branch: no certificate issued, yet stored status/notification/
        // audit disagreed. Normalise and accept only APPROVED / REJECTED.
        String normalizedStatus = request.getStatus() == null ? "" : request.getStatus().trim().toUpperCase();
        if (!ActionConstants.APPROVED.equals(normalizedStatus) && !ActionConstants.REJECTED.equals(normalizedStatus)) {
            throw new BadRequestExceptions("status must be APPROVED or REJECTED");
        }
    ...
    @Override
    public CursorPageResponse<CertificateRequestListResponse> getCertificateRequests(String status, String search, Long cursor, int limit) {
        log.info("[getCertificateRequests] status={} search={} cursor={} limit={}", status, search, cursor, limit);
        Pageable pageable = PageRequest.of(0, limit + 1);
        // BUG FIX: certificate statuses are stored upper case (PENDING/APPROVED/
        // REJECTED) but the shared admin verification dropdown sends lower case,
        // so the equality filter silently matched nothing and the admin saw an
        // empty list for every non-"All" selection. Normalise before querying.
        String normalizedStatus = (status == null || status.isBlank())
                ? null
                : status.trim().toUpperCase();
        // BUG FIX: the admin search box's ?search term was ignored here too.
        String term = (search == null || search.isBlank()) ? null : search.trim();
        List<UserCertificateRequest> requests;
        if (term == null) {
            requests = normalizedStatus == null
                    ? userCertificateRequestRepository.findPageFromCursor(cursor, pageable)
                    : userCertificateRequestRepository.findPageByStatusFromCursor(normalizedStatus, cursor, pageable);
        } else {
            requests = normalizedStatus == null
                    ? userCertificateRequestRepository.findPageBySearchFromCursor(term, cursor, pageable)
                    : userCertificateRequestRepository.findPageByStatusAndSearchFromCursor(normalizedStatus, term, cursor, pageable);
        }
    ...
    // BUG FIX: role was resolved with getRoles().stream().findFirst() over an
    // unordered Set<Role>. setUserAsAdmin() ADDS the ADMIN role on top of the
    // existing USER role, so a promoted user holds two roles and findFirst()
    // returns either one at random -- roughly half the time a real admin was
    // reported as "user", the frontend cached that in localStorage, and the
    // admin UI vanished until cache clear. Always prefer ADMIN when present.
    private String resolvePrimaryRole(java.util.Set<Role> roles) {
    ...
    private UserResponse toResponse(User user, UserProfile profile) {
        var roleName = resolvePrimaryRole(user.getRoles());
